...
export default function Catalogo() {
  ...
  async function loadCategories() {
    try {
      const res = await api.get<Category[] | ApiCollection<Category>>("/categories", {
        auth: false,
      })
  ...
  async function loadBooks() {
    ...
    try {
      ...
      const res = await api.get<any>(`/books?${qs}`, {
      ...
      if (res.data && Array.isArray(res.data.data)) {
        setPageData({
          data: res.data.data,
          current_page: res.data.current_page ?? res.meta?.current_page ?? 1,
          last_page: res.data.last_page ?? res.meta?.last_page ?? 1,
          per_page: res.data.per_page ?? res.meta?.per_page ?? res.data.data.length,
          total: res.data.total ?? res.meta?.total ?? res.data.data.length,
        })
      ...
      <div className="mx-auto max-w-6xl px-4 py-10">
        ...
        <div className="grid gap-6 md:grid-cols-[260px_1fr]">
          <aside className="h-fit rounded-2xl border border-[#5c3317]/40 bg-gradient-to-b from-[#3b1f0e]/50 to-[#1a0f06]/70 p-5">
            ...
            <p className="mt-0.5 font-serif text-base font-bold text-[#f5ede0]">
              Buscar y filtrar
            </p>
            ...
            <div className="mt-5 space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-full border border-[#5c3317]/50 bg-[#1a0f06] px-4 py-2 text-sm text-[#f5ede0] outline-none placeholder:text-[#f5ede0]/25 focus:border-[#c8922a]/60 focus:bg-[#f5ede0]/8 transition"
                />
              </div>
            ...
            <div className="flex items-center justify-between gap-4 mb-6">
              ...
              {pageData && pageData.last_page > 1 && (
                <span className="text-xs text-[#f5ede0]/35">
                  Pág.{" "}
                  <span className="text-[#f5ede0]/60 font-semibold">
                  ...
                  {Array.from({ length: pageData.last_page }, (_, i) => i + 1)
                    ...
                    .map((n, idx) =>
                      n === "…" ? (
                        <span
                          key={`ellipsis-${idx}`}
                          className="px-1 text-[#f5ede0]/25 text-sm"
                        >
                          …
                        </span>
                      ) : (
